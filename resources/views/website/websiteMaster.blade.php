
<!doctype html>
<html class="no-js" lang="">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <title>Home Version One || Sellshop</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">
		<!-- favicon -->
        <link rel="shortcut icon" type="image/x-icon" href="img/favicon.ico">

        <link rel="apple-touch-icon" href="apple-touch-icon.png">
        <!-- Place favicon.ico in the root directory -->
		<!-- google fonts -->
		<link href='https://fonts.googleapis.com/css?family=Lato:400,900,700,300' rel='stylesheet' type='text/css'>
		<link href='https://fonts.googleapis.com/css?family=Bree+Serif' rel='stylesheet' type='text/css'>
		<!-- all css here -->
		<!-- bootstrap v3.3.6 css -->
        <link rel="stylesheet" href="{{asset('website/css/bootstrap.min.css')}}">
		<!-- animate css -->
        <link rel="stylesheet" href="{{asset('website/css/animate.css')}}">
		<!-- pe-icon-7-stroke -->
		<link rel="stylesheet" href="{{asset('website/css/materialdesignicons.min.css')}}">
		<!-- pe-icon-7-stroke -->
		<link rel="stylesheet" href="{{asset('website/css/jquery.simpleLens.css')}}">
		<!-- jquery-ui.min css -->
        <link rel="stylesheet" href="{{asset('website/css/jquery-ui.min.css')}}">
		<!-- meanmenu css -->
        <link rel="stylesheet" href="{{asset('website/css/meanmenu.min.css')}}">
		<!-- nivo.slider css -->
        <link rel="stylesheet" href="{{asset('website/css/nivo-slider.css')}}">
		<!-- owl.carousel css -->
        <link rel="stylesheet" href="{{asset('website/css/owl.carousel.css')}}">
		<!-- style css -->
		<link rel="stylesheet" href="{{asset('website/css/style.css')}}">
		<!-- responsive css -->
        <link rel="stylesheet" href="{{asset('website/css/responsive.css')}}">
		<!-- modernizr js -->
       <script src="{{asset('website/js/vendor/modernizr-2.8.3.min.js')}} "></script>
    </head>
    <body>

        <div id="app">
        <!--[if lt IE 8]>
            <p class="browserupgrade">You are using an <strong>outdated</strong> browser. Please <a href="http://browsehappy.com/">upgrade your browser</a> to improve your experience.</p>
        <![endif]-->

        <!-- header section start -->

        <!-- header section end -->

        <website-component> </website-component>
        <!-- slider-section-start -->


        <!-- footer section start -->

        <!-- footer section end -->
    </div>
		<!-- all js here -->
		<!-- jquery latest version -->
        <script src="{{asset('js/app.js')}}"defer></script>
       <script src="{{asset('website/js/vendor/jquery-1.12.3.min.js')}} "></script>
		<!-- bootstrap js -->
       <script src="{{asset('website/js/bootstrap.min.js')}} "></script>
		<!-- owl.carousel js -->
       <script src="{{asset('website/js/owl.carousel.min.js')}} "></script>
		<!-- meanmenu js -->
       <script src="{{asset('website/js/jquery.meanmenu.js')}} "></script>
		<!-- countdown JS -->
       <script src="{{asset('website/js/countdown.js')}} "></script>
		<!-- nivo.slider JS -->
       <script src="{{asset('website/js/jquery.nivo.slider.pack.js')}} "></script>
		<!-- simpleLens JS -->
       <script src="{{asset('website/js/jquery.simpleLens.min.js')}} "></script>
		<!-- jquery-ui js -->
       <script src="{{asset('website/js/jquery-ui.min.js')}} "></script>
		<!-- load-more js -->
       <script src="{{asset('website/js/load-more.js')}} "></script>
		<!-- plugins js -->
       <script src="{{asset('website/js/plugins.js')}} "></script>
		<!-- main js -->
       <script src="{{asset('website/js/main.js')}} "></script>
    </body>
</html>
